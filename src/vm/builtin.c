...
static void list(sl_ctx_t *ctx)
{
    ...
    vector_push(ctx->stack, sl_value_array(arr));
}
...
static void dict(sl_ctx_t *ctx)
{
    ...
    if (n == 0)
        vector_push(ctx->stack, sl_value_dict(NULL, NULL));
    else
    {
        ...
        {
            vector_push(values, sl_value_get(i, v));
        }

        vector_push(ctx->stack, sl_value_dict(keys, values));
    }
}
...
static void len(sl_ctx_t *ctx)
{
    ...
    {
        vector_push(ctx->stack, sl_value_number((double)strlen(v->string)));
        return;
    }
    else if (v->type == SL_VALUE_ARRAY)
    {
        vector_push(ctx->stack, sl_value_number((double)vector_size(v->array)));
        return;
    }
